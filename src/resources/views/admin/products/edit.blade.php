        ...
        <div class="rounded-lg bg-white p-6 shadow-sm">
            <form method="POST" action="{{ route('admin.products.update', $product) }}" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="url" id="prd_picture" name="prd_picture" value="{{ old('prd_picture', $product->prd_picture) }}" placeholder="https://example.com/image.jpg" class="w-full rounded-lg border border-slate-300 bg-white px-4 py-2 text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-[#1a73e8] focus:ring-2 focus:ring-blue-100">
                    @if ($product->prd_picture)
                        <div class="mt-3">
                            <img src="{{ $product->prd_picture }}" alt="{{ $product->prd_name }}" class="h-20 w-20 rounded object-cover">
                        </div>
                    @endif
                    @error('prd_picture')
                        <span class="mt-1 text-sm text-red-600">{{ $message }}</span>
