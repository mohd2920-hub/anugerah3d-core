    ...
    <div class="grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
            ...
            <section class="overflow-hidden rounded-lg bg-white shadow-sm ring-1 ring-slate-200/70">
                ...
                <div class="hidden overflow-x-auto md:block">
                    <table class="w-full text-sm">
                        <thead class="bg-slate-50 text-xs text-slate-600"><tr><th class="px-5 py-3 text-left font-semibold">Product</th><th class="px-5 py-3 text-right font-semibold">Quantity</th><th class="px-5 py-3 text-right font-semibold">Unit price</th><th class="px-5 py-3 text-right font-semibold">Discount</th><th class="px-5 py-3 text-right font-semibold">Line total</th></tr></thead>
                        <tbody class="divide-y divide-slate-200">@foreach ($sale->items as $item)<tr><td class="px-5 py-4"><p class="font-semibold text-slate-900">{{ $item->product_name }}</p><p class="mt-1 font-mono text-xs text-slate-500">{{ $item->product_code }}</p></td><td class="px-5 py-4 text-right">{{ $item->quantity }}</td><td class="px-5 py-4 text-right">RM {{ number_format((float) $item->unit_price, 2) }}</td><td class="px-5 py-4 text-right"><p>{{ number_format((float) $item->discount_percentage, 2) }}%</p><p class="mt-1 text-xs text-emerald-700">- RM {{ number_format((float) $item->discount_amount, 2) }}</p></td><td class="px-5 py-4 text-right font-semibold">RM {{ number_format((float) $item->line_total, 2) }}</td></tr>@endforeach</tbody>
                    </table>
                </div>
                <div class="divide-y divide-slate-100 md:hidden">@foreach ($sale->items as $item)<div class="p-4"><div class="flex justify-between gap-3"><div><p class="font-semibold text-slate-900">{{ $item->product_name }}</p><p class="mt-1 font-mono text-xs text-slate-500">{{ $item->product_code }}</p></div><p class="font-semibold text-slate-950">RM {{ number_format((float) $item->line_total, 2) }}</p></div><p class="mt-2 text-xs text-slate-500">{{ $item->quantity }} × RM {{ number_format((float) $item->unit_price, 2) }}</p>@if ((float) $item->discount_amount > 0)<p class="mt-1 text-xs font-medium text-emerald-700">Discount {{ number_format((float) $item->discount_percentage, 2) }}% · - RM {{ number_format((float) $item->discount_amount, 2) }}</p>@endif</div>@endforeach</div>
                <div class="border-t border-slate-200 bg-slate-50 px-5 py-4">
                    <dl class="ml-auto max-w-sm space-y-2 text-sm"><div class="flex justify-between gap-4"><dt class="text-slate-500">Gross total</dt><dd class="font-medium text-slate-800">RM {{ number_format($itemSummary["gross_total"], 2) }}</dd></div><div class="flex justify-between gap-4"><dt class="text-slate-500">Total discount</dt><dd class="font-medium text-emerald-700">- RM {{ number_format($itemSummary["discount_total"], 2) }}</dd></div><div class="flex justify-between gap-4 border-t border-slate-200 pt-2"><dt class="font-semibold text-slate-900">Sale total</dt><dd class="font-semibold text-slate-950">RM {{ number_format((float) $sale->total_amount, 2) }}</dd></div></dl>
                </div>
            </section>
        ...
        <aside class="space-y-5">
            ...
            <section class="rounded-lg bg-white p-5 shadow-sm ring-1 ring-slate-200/70">
                ...
                <dl class="mt-4 space-y-3 text-sm"><div><dt class="text-xs font-semibold uppercase text-slate-500">Signed in</dt><dd class="mt-1 text-slate-800">{{ $sale->posSession->signed_in_at->format('d M Y, h:i A') }}</dd></div><div><dt class="text-xs font-semibold uppercase text-slate-500">Session expiry</dt><dd class="mt-1 text-slate-800">{{ $sale->posSession->expires_at->format('d M Y, h:i A') }}</dd></div><div><dt class="text-xs font-semibold uppercase text-slate-500">Last updated</dt><dd class="mt-1 text-slate-800">{{ $sale->updated_at->format('d M Y, h:i A') }}</dd></div></dl>
            </section>
        </aside>
    </div>
</div>
