...
@endadminRoute
    ...
    <form method="POST" action="{{ $sale ? route('admin.sales.preview', $sale) : route('admin.sales.preview-missing', $operation) }}" class="space-y-5" data-sale-correction>
        ...
        <div class="grid gap-4 rounded-xl bg-white p-5 ring-1 ring-slate-200 sm:grid-cols-2">
            ...
            <label class="text-sm font-medium">Sale time<input required type="datetime-local" step="1" name="sold_at" value="{{ old('sold_at', ($sale?->sold_at ?? $operation->closed_at ?? now())->format('Y-m-d\TH:i:s')) }}" class="mt-1 w-full rounded-lg border-slate-300"></label>
