<div data-correction-item class="grid items-end gap-3 rounded-lg bg-slate-50 p-3 sm:grid-cols-4">
    <label class="text-xs font-semibold">Product<select required name="items[{{ $index }}][product_id]" class="mt-1 w-full rounded-lg border-slate-300 text-sm"><option value="">Select product</option>@foreach ($products as $product)@if (!$product->discontinued_at || (string) $item['product_id'] === (string) $product->id)<option value="{{ $product->id }}" @selected((string) $item['product_id'] === (string) $product->id)>{{ $product->prd_name }} ({{ $product->prd_code }})</option>@endif@endforeach</select></label>
