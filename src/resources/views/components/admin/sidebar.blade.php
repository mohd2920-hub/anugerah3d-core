@php
    $customerCatalogueReady = \Illuminate\Support\Facades\Schema::hasTable('customer_orders');
    ...
    if ($pageTitle === "") {
        $pageTitle = match (true) {
            ...
            request()->routeIs("admin.customer-orders.*") => "Customer Orders",
            request()->routeIs("admin.orders.*") => "Orders",
            request()->routeIs("admin.salary-management.*") => "Salary Management",
...
@if($customerCatalogueReady || \App\Support\AdminAccess::canRoute($adminUser, 'admin.dashboard'))
<a href="{{ $customerCatalogueReady ? route('admin.customer-orders.index') : route('admin.dashboard').'#pembelian-terus-pelanggan' }}" @class(['rounded-lg px-3 py-2.5 leading-5 transition','bg-white text-[#1a73e8]' => request()->routeIs('admin.customer-orders.*'),'text-slate-300 hover:bg-white/10 hover:text-white' => !request()->routeIs('admin.customer-orders.*')])>Pembelian Terus Pelanggan</a>
@else
<span class="rounded-lg px-3 py-2.5 leading-5 text-slate-400">Pembelian Terus Pelanggan <span class="block text-xs">Menunggu pengaktifan</span></span>
@endif
@endadminRoute
...
@endadminRoute
                @adminRoute('admin.salary-management.index')
<a href="{{ route('admin.salary-management.index') }}" @class(['rounded-lg px-3 py-2.5 leading-5 transition', 'bg-white text-[#1a73e8] shadow-sm' => request()->routeIs('admin.salary-management.*'), 'text-slate-300 hover:bg-white/10 hover:text-white' => !request()->routeIs('admin.salary-management.*')])>Salary Management</a>
@endadminRoute
...
@endadminRoute
                ...
                <details class="group/sys" {{ $systemActive ? "open" : "" }}>
                    ...
                    <div class="mt-1 grid gap-1 border-l border-white/10 pl-3">
                        @adminRoute('admin.system.manage-data')
...
@endadminRoute
                        @adminRoute('admin.system.staff.index')
<a href="{{ route('admin.system.staff.index') }}" class="rounded-lg px-3 py-2 text-sm hover:bg-white/10">User Management</a>
@endadminRoute
                        @adminRoute('admin.system.roles.index')
<a href="{{ route('admin.system.roles.index') }}" class="rounded-lg px-3 py-2 text-sm hover:bg-white/10">Roles &amp; Permissions</a>
@endadminRoute
...
<aside {{ $attributes->merge(["class" => "hidden border-b border-[#273154] bg-[#111827] text-white lg:flex lg:border-b-0 lg:border-r"]) }}>
    <div class="flex h-full w-full flex-col px-5 py-5">
        <a href="{{ route(\App\Support\AdminAccess::landingRoute($adminUser)) }}" class="flex items-center gap-3" aria-label="Anugerah3D admin dashboard">
            <x-brand-logo />
            <span class="min-w-0">
                <span class="block truncate text-sm font-semibold">Anugerah3D</span>
                <span class="block truncate text-xs text-blue-200/80">Admin Operations</span>
