<div data-checkout-modal class="fixed inset-0 z-50 hidden justify-center bg-[#f7f9fa] sm:items-center sm:bg-slate-950/45 sm:p-5 sm:backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="checkout-title">
    <form data-checkout-form action="{{ route('agent.orders.store') }}" method="post" data-history-url="{{ route('agent.history') }}" class="flex h-full w-full max-w-xl flex-col overflow-hidden bg-[#f7f9fa] sm:h-[92vh] sm:rounded-[2rem] sm:shadow-2xl">
        @csrf
