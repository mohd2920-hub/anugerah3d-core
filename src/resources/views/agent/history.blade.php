            ...
            <div class="flex-1 space-y-4 overflow-y-auto p-4 pb-8">
                ...
                <section class="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm"><div class="flex items-center justify-between"><h3 class="text-sm font-extrabold text-[#17324d]">Products</h3><span class="text-[10px] font-bold text-slate-400">{{ count($order['products']) }} types</span></div><div class="mt-3 divide-y divide-slate-100">
                    @foreach ($order['products'] as $product)
                        <article class="flex items-start gap-3 py-3 first:pt-0 last:pb-0"><span class="grid h-10 w-10 flex-none place-items-center rounded-xl bg-slate-100 text-[9px] font-black text-slate-500">{{ $product['quantity'] }}×</span><div class="min-w-0 flex-1"><div class="flex items-center gap-2"><p class="truncate text-xs font-extrabold text-[#17324d]">{{ $product['name'] }}</p>@if($product['preorder'])<span class="flex-none rounded-full bg-orange-50 px-1.5 py-0.5 text-[7px] font-extrabold uppercase text-[#d95419]">Pre-order</span>@endif</div><p class="mt-1 font-mono text-[9px] text-slate-400">{{ $product['code'] }}</p>@if(($product['clicker_character_count'] ?? 0) > 0)@php $clickerCharacters = collect(mb_str_split((string) ($product['clicker_characters'] ?? '')))->filter(fn ($character) => trim((string) $character) !== ''); @endphp<div class="mt-1.5"><p class="inline-block rounded-full bg-slate-100 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wide text-slate-600">Characters ({{ $product['clicker_character_count'] }})</p><div class="mt-1 flex flex-nowrap items-center gap-1 overflow-x-auto pb-0.5">@forelse($clickerCharacters as $character)<span class="inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-md border border-orange-200 bg-[linear-gradient(180deg,#fff7ed,#ffedd5)] text-[10px] font-black text-[#c2410c] shadow-sm">{{ strtoupper($character) }}</span>@empty<span class="text-xs font-semibold text-slate-400">-</span>@endforelse</div></div>@endif<p class="mt-1 text-[10px] text-slate-500">{{ $product['quantity'] }} × RM {{ number_format($product['price'], 2) }}</p></div><p class="flex-none text-xs font-black text-[#e7682b]">RM {{ number_format($product['quantity'] * $product['price'], 2) }}</p></article>
                    @endforeach
