        ...
        <div class="mt-3 space-y-3" data-pos-items>
            @foreach ($currentItems as $index => $currentItem)
                <div class="rounded-2xl border border-slate-200 p-3" data-pos-item>
                        ...
                        <div class="relative min-w-0 flex-1">
                            ...
                            <select name="items[{{ $index }}][product_id]" class="sr-only" data-pos-product>
                                ...
                                @foreach ($products as $product)
                                    <option value="{{ $product->id }}" data-type="{{ $product->product_type }}" data-price="{{ $product->price_selling }}" data-default-discount="{{ $product->agent_discount_default }}" data-code="{{ $product->prd_code }}" data-name="{{ $product->prd_name }}" data-search="{{ Str::lower($product->prd_code.' '.$product->prd_name) }}" data-image="{{ $productPictureUrl($product) }}" @selected((string) $currentItem['product_id'] === (string) $product->id)>{{ $product->prd_code }} · {{ $product->prd_name }} · RM {{ number_format((float) $product->price_selling, 2) }}{{ ($product->discontinued_at ?? null) ? ' · Stok terakhir: '.$product->prd_balance : '' }}</option>
                                @endforeach
                            </select>
                    ...
                    <div data-pos-clicker data-initial='@json($currentItem)' class="mt-3 hidden space-y-2 rounded-xl bg-slate-50 p-3"></div>
...
<template data-pos-item-template>
    <div class="rounded-2xl border border-slate-200 p-3" data-pos-item>
        <div class="flex gap-2"><div class="relative min-w-0 flex-1"><input type="search" value="" placeholder="Choose product" autocomplete="off" class="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm" data-pos-product-search><select class="sr-only" data-pos-product><option value="">Choose product</option>@foreach ($products as $product)<option value="{{ $product->id }}" data-type="{{ $product->product_type }}" data-price="{{ $product->price_selling }}" data-default-discount="{{ $product->agent_discount_default }}" data-code="{{ $product->prd_code }}" data-name="{{ $product->prd_name }}" data-search="{{ Str::lower($product->prd_code.' '.$product->prd_name) }}" data-image="{{ $productPictureUrl($product) }}">{{ $product->prd_code }} · {{ $product->prd_name }} · RM {{ number_format((float) $product->price_selling, 2) }}{{ ($product->discontinued_at ?? null) ? ' · Stok terakhir: '.$product->prd_balance : '' }}</option>@endforeach</select><div class="absolute left-0 right-0 top-full z-20 mt-1 hidden max-h-56 overflow-auto rounded-xl border border-slate-200 bg-white shadow-lg" data-pos-product-list></div></div><button type="button" data-remove-pos-item class="grid h-11 w-11 place-items-center rounded-xl bg-red-50 text-red-600">×</button></div>
        <div data-pos-clicker class="mt-3 hidden space-y-2 rounded-xl bg-slate-50 p-3"></div><div class="mt-2 grid grid-cols-2 gap-2"><div><label class="text-[10px] font-bold uppercase text-slate-400">Quantity</label><div class="mt-1 flex items-center gap-2"><button type="button" data-pos-qty-minus class="grid h-11 w-11 place-items-center rounded-xl border border-slate-200 bg-slate-50 text-lg font-black text-[#17324d]">-</button><input type="number" value="1" min="1" max="9999" required class="h-11 min-w-0 flex-1 rounded-xl border border-slate-200 px-3 text-center text-base font-bold" data-pos-quantity><button type="button" data-pos-qty-plus class="grid h-11 w-11 place-items-center rounded-xl border border-slate-200 bg-slate-50 text-lg font-black text-[#17324d]">+</button></div></div><button type="button" class="rounded-xl bg-slate-50 px-3 py-2 text-left transition active:bg-orange-50" data-open-pos-discount><span class="block text-[10px] font-bold uppercase text-slate-400">Line total</span><span class="mt-1 block text-sm font-extrabold text-[#17324d]" data-pos-line-total>RM 0.00</span><span class="mt-1 block text-[10px] font-bold text-[#d95419]" data-pos-discount-label>Discount RM 0.00</span></button><input type="hidden" value="" data-pos-discount data-custom-discount="false"></div>
    </div>
</template>
